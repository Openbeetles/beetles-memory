...
linux_desktop_profile_forwarding=(
  "crates/adapter/Cargo.toml|bm-sdk/profile-desktop-linux-embedded-sdk"
  "crates/entry/Cargo.toml|bm-adapter/profile-desktop-linux-embedded-sdk"
  "crates/cli/Cargo.toml|bm-entry/profile-desktop-linux-embedded-sdk"
  "crates/http/Cargo.toml|bm-adapter/profile-desktop-linux-embedded-sdk"
  "crates/http/Cargo.toml|bm-entry?/profile-desktop-linux-embedded-sdk"
  "crates/wss/Cargo.toml|bm-adapter/profile-desktop-linux-embedded-sdk"
  "crates/mcp/Cargo.toml|bm-adapter/profile-desktop-linux-embedded-sdk"
  "crates/a2a/Cargo.toml|bm-adapter/profile-desktop-linux-embedded-sdk"
)

for row in "${linux_desktop_profile_forwarding[@]}"; do
  manifest="${row%%|*}"
  forwarding="${row#*|}"
  if ! grep -F -- "$forwarding" "$manifest" >/dev/null; then
    fail "$manifest must forward the Linux desktop embedded profile through $forwarding"
  fi
done
...
if rg -n 'profile-esp-|profile-desktop-(macos|linux|windows)-embedded-sdk|profile-linux-device-standalone-memory' crates/llm-gateway/Cargo.toml >/tmp/bm-llm-gateway-profile-hit.$$; then
...
compact_checks=(
  ...
  "bm-entry profile-desktop-linux-embedded-sdk"
  ...
  "bm-http profile-desktop-linux-embedded-sdk"
