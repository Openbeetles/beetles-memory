...
`HostUi` is only the host-presentable redacted disclosure view. It is not a chat panel, history manager, pagination direction, index owner, or authorization capability. Catalog and timeline remain under `transcript_replay`; indexed search and activity have independent `transcript_search` and `transcript_activity` switches. Platform capability snapshots expose them under `beetle-memory.platform.capability.v5`.
...
The published v0.7.0 preserves CTQ1 InMemory/File/SQLite atomic head/catalog/time/search closure under Store v13; 0.8.0 uses Store v14 and retains those contracts. Older Store generations have no compatibility reader or migration path; development data must be explicitly recreated. Repair/archive closure, private-authority exact-zero, and strict regression evidence remain required. This does not claim real-data handling, crates.io or hosted Release publication, or runtime/UAT execution.

The 0.7.0 turn-intake correction preserves identical text from distinct turns, with message identity bound to the conversation owner, subject, turn, and ordinal rather than a bounded session count. Exact turn retries remain idempotent; altered canonical payloads conflict. The correction changes neither Store schema nor replay cursors, and does not reconstruct messages omitted by older code or perform a data migration. For protocol history normalization before commit, see [the API contract](./api.md#memory-evidence-system).
...
Runtime Skill records and their scope manifests are protected local procedural owners. Public memory-space archives never carry their raw records or authoritative events, regardless of the requested private-material policy. Import preserves an existing local Runtime Skill owner exactly and cannot clone a Runtime Skill into another Store or subject; controlled full-snapshot recovery remains a separate owner-only mechanism. In 0.8.0, this protection also excludes internal procedural source/job audit events: import keeps the target's existing protected audit and atomically records locally derived source changes, without transplanting source audit authority.

In 0.8.0 (Store v14; v0.7.0 uses v13), restoring the exact public projection of an existing transcript preserves that Store's actual record and local learning evidence. It does not import evidence or producer authority into a fresh Store. An archive that omits a retained local learning source returns typed `MemorySpaceImportConflict::ProtectedTranscriptSourceMissing`; a differing existing turn, including an attempt to restore raw content over `RawDeleted`, returns `ExistingTranscriptDiffers`. Both leave the Store unchanged. Keep the target's current source projections in the archive; perform intentional source withdrawal through the explicit transcript lifecycle API and use an archive consistent with that terminal state. Import never silently performs permanent learning withdrawal. Ordinary omitted transcripts without learning evidence or retained learning references still follow scoped replacement. Public-only replacement preserves the shared cursor keyring used by other subjects; a full transcript restore creates a new cursor incarnation.
