...
<div class="modal-host">
  ...
  <div class="modal skill-editor-modal" role="dialog" aria-modal="true" aria-labelledby="skill-editor-title" tabindex="-1" use:accessibleDialog={{ onClose }} transition:modalPanel>
    ...
    <form class="modal-body" onsubmit={onSubmit}>
      <div class="skill-form-grid">
        <label>
          ...
          <input data-dialog-autofocus value={form.title} oninput={(event) => onFieldChange("title", (event.currentTarget as HTMLInputElement).value)} required autocomplete="off" disabled={loading} />
        </label>
        <label>
