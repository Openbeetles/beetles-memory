...
<div class="modal" role="dialog" aria-modal="true" aria-labelledby="add-device-title" transition:modalPanel>
  ...
  <form class="modal-body" onsubmit={onSubmit}>
    <label>
      ...
      <input value={label} oninput={(event) => onLabelChange((event.currentTarget as HTMLInputElement).value)} placeholder={t.addDevice.namePlaceholder} required autocomplete="off" disabled={loading} />
    </label>
