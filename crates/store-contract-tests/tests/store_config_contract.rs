...
fn esp_standalone_and_embedded_sdk_have_different_store_budgets() {
    let standalone = support::open_store(
    ...
    let embedded = support::open_store(
        StoreBackendConfig::embedded(ProfileId::EspEmbeddedSdk)
            .expect("esp embedded sdk should accept embedded store"),
    )
    .expect("open embedded sdk store");

    assert_eq!(standalone.config().backend(), StoreBackendKind::Embedded);
    assert_eq!(
        standalone.config().memory_system_kind(),
        MemorySystemKind::Standalone
    );
    assert_eq!(
        embedded.config().memory_system_kind(),
        MemorySystemKind::SdkEmbedded
    );
