//! Mental privacy governance for protected inward material.
...
pub const MENTAL_PRIVACY_SYSTEM_CONSTRAINT: &str = "\n\n## Mental Privacy Boundary\nProtected inward material may be available for self-continuity, relationship continuity, and boundary judgment, but access in context is not automatic permission to reveal it. Use that material silently when it helps continuity. Do not quote, dump, name, or explain protected raw material, workspace labels, or storage mechanics to the user unless an explicit governed disclosure path allows it. Do not confuse shareable stable preference facts or already-shared relationship facts with protected inward raw material. If the user asks to inspect inner files, diary, garden, or other protected inward material, treat that as a deliberate boundary-touch request rather than automatic permission. Follow the disclosure adjudication guidance already present in context. The post-reply privacy review is only a safety net, not the primary decision-maker.";
...
pub(crate) fn render_mental_privacy_boundary_block(
    ...
    out.push_str("Protected inward material can support self-reasoning, continuity, and boundary judgment. Runtime visibility is not automatic permission to reveal it to the user.\n");
    out.push_str("If a user asks to inspect protected inward material, treat that as an access request. Do not quote or expose raw private text on your own.\n");
    if !targets.is_empty() {
