...
fn material(
    ...
    AgentToolExperienceRevisionMaterialV2::build(
        ...
        "pdf.extract",
        "schema-pdf-v1",
        "extract_pdf_text_for_release_notes",
        revision,
        "PDF release-note extraction",
        "Prefer the governed extraction path.",
...
fn revision_history_and_locator_are_canonical_and_tamper_evident() {
    ...
    let retained_first =
        AgentToolExperienceRetainedRevisionDigestV2::from_material(&first).expect("retained first");
    ...
    let locator = AgentToolExperienceOwnerLocatorV2::from_material(&second);
    ...
    assert!(serde_json::from_value::<AgentToolExperienceOwnerLocatorV2>(encoded).is_ok());
    ...
    assert!(AgentToolExperienceOwnerLocatorV2::try_new(
        " space-a",
        scope("subject-a"),
...
fn head_and_manifest_validate_the_exact_owner_closure() {
    let first = material("space-a", "subject-a", 1, None);
    let retained = AgentToolExperienceRetainedRevisionDigestV2::from_material(&first)
    ...
    tampered.usage_guidance.push_str(" tampered");
    assert!(
...
fn head_cannot_drop_the_initial_revision_from_retained_history() {
    ...
    let retained_second = AgentToolExperienceRetainedRevisionDigestV2::from_material(&second)
        .expect("retained second");

    assert!(AgentToolExperienceOwnerHeadV2::build(
        "space-a",
        scope("subject-a"),
        second.owner_ref,
        2,
