...
use bm_store::{FileStore, InMemoryStore, MemoryStore, StoreResult};
use std::fs;
use std::path::{Path, PathBuf};
use std::time::{SystemTime, UNIX_EPOCH};

#[test]
fn in_memory_store_keeps_procedural_metadata() -> StoreResult<()> {
    let mut store = InMemoryStore::default();
    let inserted = store.insert(record())?;

    assert_eq!(
        inserted.meta.procedural.as_ref().map(|meta| meta.origin),
...
#[test]
fn file_store_replays_procedural_metadata() -> StoreResult<()> {
    let root = TempStoreRoot::new("file_store_replays_procedural_metadata");
    ...
    assert_eq!(
        records[0].meta.procedural.as_ref().map(|meta| meta.state),
        Some(ProceduralSkillState::Active)
    );
    Ok(())
}
